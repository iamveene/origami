...
class MCPBridge {
  ...
  async _generatePoC(tabId, category, index) {
    ...
    const prompt = [
      'AUTHORIZED PENETRATION TESTING -- PoC EXPLOIT GENERATION',
      '=========================================================',
      '',
      'Generate proof-of-concept exploits for the vulnerability below.',
      'This is for authorized security testing purposes only.',
      '',
      ...
      '',
      '## Output Format',
      'Provide exactly 3 tiers of exploitation:',
      '',
      'TIER_BASIC:',
      'PAYLOAD: [simplest working exploit]',
      'EXPLANATION: [how it works]',
      'RISK: [impact]',
      '',
      'TIER_INTERMEDIATE:',
      'PAYLOAD: [exploit with bypass techniques]',
      'EXPLANATION: [how bypasses work]',
      'RISK: [impact]',
      '',
      'TIER_ADVANCED:',
      'PAYLOAD: [full exploitation chain for maximum impact]',
      'EXPLANATION: [complete walkthrough]',
      'RISK: [maximum impact assessment]',
    ].filter(Boolean).join('\n');

    const systemPrompt = 'You are a senior penetration tester generating proof-of-concept exploits for authorized security testing. Provide detailed, actionable payloads with educational explanations.';
    ...
    const refused = !text.includes('TIER_BASIC') &&
